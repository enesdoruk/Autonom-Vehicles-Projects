...
{
  ros::init(argc, argv, "tfmini_ros_right");
  ros::NodeHandle nh("~");
  ...
  nh.param("serial_port", portName, std::string("/dev/ttyUSB2"));
  nh.param("baud_rate", baud_rate, 115200);

  tfmini_obj = new benewake::TFmini(portName, baud_rate);
  ros::Publisher pub_range = nh.advertise<sensor_msgs::Range>(id, 10, true);
  ...
  while(ros::master::check() && ros::ok())
  {
    ...
    std::cout << "Front Radar Distance = " << dist << std::endl;
  }
